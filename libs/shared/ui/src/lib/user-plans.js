...
export function UserPlans() {
  ...
  const plans = [
    { id: 1, name: 'Basic', price: '$0/mo', features: ['Real-time Quotes', '1 Demo Account'] },
    { id: 2, name: 'Pro', price: '$49/mo', features: ['Premium Support', 'Unlimited Accounts', 'API Access'] },
    { id: 3, name: 'Enterprise', price: '$199/mo', features: ['Dedicated Account Manager', 'Custom API Limits'] },
  ];

  const handleSubscribe = (name) => {
    alert(`Successfully upgraded to the ${name} plan!`);
    setCurrentPlan(name);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 md:space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-card p-4 md:p-6 rounded-lg shadow-sm border border-border gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-foreground">My Subscription</h1>
          <p className="text-sm md:text-base text-muted-foreground mt-1">
            Manage your <strong className="text-primary">{theme.tenantName}</strong> membership tier.
          </p>
        </div>
        <div className="text-right p-3 bg-primary/10 rounded-md border border-primary/20">
          ...
          <p className="text-xl font-extrabold text-foreground tracking-tight">{currentPlan}</p>
        </div>
      </div>
        ...
        {plans.map((plan) => (
          <Card 
            ...
            className={`flex flex-col h-full border p-6 shadow-md transition-shadow hover:shadow-lg ${
              currentPlan === plan.name ? 'border-primary ring-2 ring-primary/50' : 'border-border'
            }`}
          >
