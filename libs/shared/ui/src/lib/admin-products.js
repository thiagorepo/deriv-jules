...
export function AdminProducts() {
  ...
  const [products, setProducts] = useState([
    { id: 1, name: 'Advanced AI Bot', price: '$499.00', status: 'Active', sales: 124 },
    { id: 2, name: 'Crypto Signals', price: '$99/mo', status: 'Active', sales: 892 },
    { id: 3, name: 'Forex Masterclass', price: '$199.00', status: 'Draft', sales: 0 },
  ]);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-card p-4 md:p-6 rounded-lg shadow-sm border border-border gap-4">
        <div>
          ...
          <p className="text-sm md:text-base text-muted-foreground mt-1">
            Manage the marketplace offerings for <strong className="text-primary">{theme.tenantName}</strong>.
          </p>
        </div>
            ...
            <tbody className="divide-y divide-border">
              {products.map((product) => (
                <tr key={product.id} className="bg-card hover:bg-muted/20 transition-colors">
                  <td className="px-4 md:px-6 py-4 font-semibold text-foreground">{product.name}</td>
                  <td className="px-4 md:px-6 py-4 tabular-nums">{product.price}</td>
                  ...
                  <td className="px-4 md:px-6 py-4 tabular-nums">{product.sales}</td>
                  <td className="px-4 md:px-6 py-4 text-right">
                    <Button variant="secondary" className="text-xs py-1 px-3 min-h-0">Edit</Button>
                  </td>
                </tr>
              ))}
            </tbody>
