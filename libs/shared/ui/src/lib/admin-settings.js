...
export function AdminSettings() {
  ...
  const [webhookUrl, setWebhookUrl] = useState('https://api.example.com/webhooks/stripe');
  ...
  const handleSave = (e) => {
    ...
    setTimeout(() => {
      setLoading(false);
      setSuccessMessage('Tenant settings updated successfully. Changes will apply on the next page reload.');
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 md:space-y-8">
      <div className="bg-card p-4 md:p-6 rounded-lg shadow-sm border border-border">
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-foreground">Tenant Settings</h1>
        <p className="text-sm md:text-base text-muted-foreground mt-1">
          Configure branding and third-party integrations for the <strong className="text-primary">{theme.tenantName}</strong> white-label instance.
        </p>
      </div>
      ...
      {successMessage && (
        <div className="bg-green-500/10 border border-green-500/20 text-green-500 p-4 rounded-md font-medium flex items-center justify-between shadow-sm">
          ...
          <button onClick={() => setSuccessMessage('')} className="text-green-500 hover:bg-green-500/20 p-1 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-green-500">
      ...
      <form onSubmit={handleSave} className="space-y-6 md:space-y-8">
        ...
        <Card className="space-y-4">
          <div className="border-b border-border pb-4 mb-4">
            <h2 className="text-xl font-bold text-foreground">Branding Configuration</h2>
            <p className="text-sm text-muted-foreground">Customize the look and feel of your trading platform.</p>
          </div>
        ...
        <Card className="space-y-4">
          <div className="border-b border-border pb-4 mb-4">
            ...
            <p className="text-sm text-muted-foreground">Manage connections to third-party services like Deriv and Stripe.</p>
          </div>
        ...
        <div className="flex justify-end gap-4 pt-4 border-t border-border">
          <Button variant="secondary" type="button" onClick={() => window.location.reload()}>Discard Changes</Button>
