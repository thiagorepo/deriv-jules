...
export function createServerClient(supabaseUrl, supabaseKey, cookieStore) {
        ...
        select: (query = '*') => {
          ...
          const builder = {
            ...
            then: function (resolve, reject) {
              ...
              if (table === 'tenants') {
                const mockTenants = [
                  { domain: 'localhost:3000', theme: 'dark', primaryColor: '#10b981', tenantName: 'Local Broker' },
                  { domain: 'crypto-bros.com', theme: 'dark', primaryColor: '#f59e0b', tenantName: 'Crypto Bros' },
                  { domain: 'forex-masters.net', theme: 'light', primaryColor: '#3b82f6', tenantName: 'Forex Masters' }
                ];

                let result = mockTenants;
                if (this._domainFilter) {
                  result = mockTenants.filter(t => t.domain === this._domainFilter);
                }
...
export function createBrowserClient(supabaseUrl, supabaseKey) {
  console.log(`[Supabase CSR] Initializing Browser client for URL: ${supabaseUrl}`);

  return {
    auth: {
      signInWithOAuth: async ({ provider }) => {
        console.log(`[Supabase CSR Auth] Attempting sign in with ${provider}...`);
